...
export default function DeploymentsPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <h1 className="text-2xl font-bold text-gray-900 mb-2">Deployments</h1>
      <p className="text-gray-500 mb-8">Deploy products directly to your Databricks workspace.</p>

      {/* Connection Status */}
      <div className="bg-white border border-gray-300 p-6 mb-8">
        <h2 className="text-lg font-bold text-gray-900 mb-3">Databricks Workspace</h2>
        {isConnected && connection ? (
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="w-3 h-3 bg-green-500 inline-block" />
              <span className="text-sm text-gray-700">Connected to {connection.workspaceUrl}</span>
            </div>
          ...
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-500">No workspace connected</p>
      ...
      {deployableAssets.length > 0 && (
        <div className="mb-10">
          <h2 className="text-lg font-bold text-gray-900 mb-4">Available for Deployment</h2>
            ...
            {deployableAssets.map((asset) => (
              <div key={asset.assetId} className="bg-white border border-gray-300 p-5 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">v{asset.version}</p>
                </div>
      ...
      {deployableAssets.length === 0 && (
        <div className="bg-white border border-gray-300 p-8 text-center mb-10">
          <p className="text-gray-500">No deployable products available. Purchase products to deploy them.</p>
        </div>
      )}
              ...
              <tbody>
                {history.map((record) => (
                  <tr key={record.id} className="border-b border-gray-100 last:border-0">
                    ...
                    <td className="px-4 py-3 text-gray-500 text-xs truncate max-w-[200px]">{record.workspaceUrl}</td>
                    ...
                    <td className="px-4 py-3 text-gray-500">{new Date(record.deployedAt).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
