...
export default function WishlistPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      {wishlisted.length === 0 ? (
        <div className="sharp-card bg-white p-12 text-center">
        ...
        <div className="space-y-3">
          {wishlisted.map((product) => (
            <div key={product.id} className="sharp-card bg-white p-5 flex items-center justify-between">
              <div>
                <Link
                  to={`/products/${product.slug}`}
                  className="font-bold text-gray-900 hover:text-blueprint-blue"
                >
                  {product.title}
                </Link>
